...
export default function ExportReports({ patientName = 'John Doe', patientId = 'P-12345' }: ExportReportsProps) {
    ...
    const handleExport = () => {
        // Generate and download the selected reports
        ...
        if (exportFormat === 'json') {
            const exportData = {
                ...
                reports: selectedReportData.map(r => ({
                    id: r.id,
                    name: r.name,
                    type: r.type,
                    date: r.date,
            ...
            // CSV header and rows
            const csvLines = ['ID,Name,Type,Date,Size'];
            selectedReportData.forEach(r => {
                csvLines.push(`${r.id},"${r.name}",${r.type},${r.date},${r.size}`);
            });
            content = csvLines.join('\n');
            mimeType = 'text/csv';
            extension = 'csv';
        } else {
            // For PDF, we'll create a simple HTML-based PDF-like content that can be printed
            const htmlContent = `
<!DOCTYPE html>
<html>
<head>
    <title>Health Report Export</title>
    <style>
        body { font-family: Arial, sans-serif; padding: 40px; }
        h1 { color: #0891b2; }
        .report { border: 1px solid #ddd; padding: 15px; margin: 10px 0; border-radius: 8px; }
        .label { color: #666; font-size: 12px; }
        .value { font-size: 14px; font-weight: bold; }
    </style>
</head>
<body>
    <h1>🏥 MedFlow Health Report</h1>
    <p><strong>Patient:</strong> ${patientName} (${patientId})</p>
    <p><strong>Export Date:</strong> ${new Date().toLocaleDateString()}</p>
    <hr/>
    <h2>Selected Reports (${selectedReportData.length})</h2>
    ${selectedReportData.map(r => `
    <div class="report">
        <div class="label">Report</div>
        <div class="value">${r.name}</div>
        <div class="label">Type: ${r.type} | Date: ${r.date} | Size: ${r.size}</div>
    </div>
    `).join('')}
    <hr/>
    <p style="color:#666;font-size:12px;">Generated by MedFlow AI Health Platform</p>
</body>
</html>`;
            content = htmlContent;
            mimeType = 'text/html';
        ...
        link.href = url;
        link.download = `medflow_reports_${new Date().toISOString().split('T')[0]}.${extension}`;
